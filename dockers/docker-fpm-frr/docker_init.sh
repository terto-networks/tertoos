...
if [ -z "$CONFIG_TYPE" ] || [ "$CONFIG_TYPE" == "separated" ]; then
    ...
    if [ -n "$MGMT_FRAMEWORK_CONFIG" ] && [ "$MGMT_FRAMEWORK_CONFIG" != "false" ]; then
        CFGGEN_PARAMS="$CFGGEN_PARAMS \
            ...
            -t /usr/local/sonic/frrcfgd/pathd.conf.j2,/etc/frr/pathd.conf \
        "
    else
        rm -f /etc/frr/bfdd.conf /etc/frr/ospfd.conf /etc/frr/ospf6d.conf /etc/frr/isisd.conf /etc/frr/ldpd.conf /etc/frr/pathd.conf
    fi
